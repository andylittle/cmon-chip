...
# measure the chip computer parameters if we can
    ('chip_voltage', 'REAL'),      # v
    ('chip_discharge', 'REAL'),    # ma
    ('chip_charge', 'REAL'),       # ma
    ('chip_batt_percent', 'REAL'), # percent
    ]
...
class Collector(object):
    ...
    def get_data(self, now_ts=None):
        ...
        if 'chip' in self.hardware:
            record.update(self._get_chip_info())
    ...
    @staticmethod
    def _get_chip_info():
        # get chip computer measurements
        record = dict()
        try:
            cmd = '/home/weewx/battery.sh'
            p = Popen(cmd, shell=True, stdout=PIPE)
            o = p.communicate()[0]
            for line in o.split('\n'):
                if line.startswith('Battery voltage'):
                    m = Collector._DIGITS.search(line)
                    if m:
                        record['chip_voltage'] = float(m.group())
                elif line.startswith('Battery discharge current'):
                    m = Collector._DIGITS.search(line)
                    if m:
                        record['chip_discharge'] = float(m.group())
                elif line.startswith('Battery charge current '):
                    m = Collector._DIGITS.search(line)
                    if m:
                        record['chip_charge'] = float(m.group())
                elif line.startswith('Internal temperature'):
                    m = Collector._DIGITS.search(line)
                    if m:
                        record['core_temp'] = float(m.group())
                elif line.startswith('Battery gauge'):
                    m = Collector._DIGITS.search(line)
                    if m:
                        record['chip_batt_percent'] = float(m.group())
        except (ValueError, IOError, KeyError), e:
            logerr('chip battert.sh failed: %s' % e)
        return record
